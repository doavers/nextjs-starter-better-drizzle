...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `${APP_CONFIG.meta.title} - ${t("new-password-title")}`,
    ...
    <>
      <div className="mx-auto flex w-full flex-col justify-center space-y-8 sm:w-[350px]">
        <div className="space-y-2 text-center">
          <h1 className="text-3xl font-medium">{t("new-password-title")}</h1>
          <p className="text-muted-foreground text-sm">{t("new-password-description")}</p>
        </div>
        <div className="space-y-4">
          ...
          <NewPasswordForm traceId={traceId} />
        </div>
      </div>
