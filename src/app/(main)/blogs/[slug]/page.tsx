...
import Newsletter from "@/components/main/section/blog/newsletter";
import PopularArticle from "@/components/main/section/blog/popular-article";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Use this -> const posts = getAllPosts(["title", "date", "excerpt", "coverImage", "slug"]);
  ...
  const authorName = process.env.NEXT_PUBLIC_AUTHOR_NAME ?? "Your Author Name";
...
export default async function Post({ params }: Props) {
  ...
  const post = getPostBySlug(slug, ["title", "author", "authorImage", "content", "coverImage", "date"]);

  const content = await markdownToHtml(post.content as string);

  return (
    <>
      <Breadcrumb pageName="Blog Details" parentPage={[{ pageName: "Blogs", pageLink: "/blogs" }]} />

      <section className="pt-20 pb-10 lg:pt-[120px] lg:pb-20 dark:bg-black">
        <div className="mx-auto max-w-screen-xl px-4">
          <div className="-mx-4 flex flex-wrap justify-center">
            <div className="w-full px-4">
              ...
              >
                ...
                />
                <div className="from-dark-700 absolute top-0 left-0 z-10 flex h-full w-full items-end bg-linear-to-t to-transparent">
                  <div className="flex flex-wrap items-center p-4 pb-4 sm:p-8">
                    <div className="mr-5 mb-4 flex items-center md:mr-10">
                      <div className="mr-4 h-10 w-10 overflow-hidden rounded-full">
                        <Image src={post.authorImage as string} alt="image" className="w-full" width={40} height={40} />
                      </div>
                    ...
                    <div className="mb-4 flex items-center">
                      <p className="mr-5 flex items-center text-sm font-medium text-white md:mr-6">
                        ...
                        {format(new Date(post.date as Date), "dd MMM yyyy")}
                      </p>
                ...
                <div className="w-full px-4 lg:w-4/12">
                  <div>
                    ...
                    <div className="-mx-4 mb-8 flex flex-wrap">
                      <div className="w-full px-4">
                        <h2
                          className="wow fadeInUp relative pb-5 text-2xl font-semibold text-black sm:text-[28px] dark:text-white"
                          data-wow-delay=".1s"
                        >
                          Popular Articles
                        </h2>
                      ...
                      {posts.slice(0, 3).map((blog, i) => (
                        <PopularArticle
                          key={i}
                          image={blog?.coverImage as string}
                          title={(blog?.title as string).slice(0, 30)}
                          name={blog?.author as string}
                        />
                      ))}
                    </div>
          ...
          <div className="-mx-4 flex flex-wrap">
            <div className="wow fadeInUp mt-14 w-full px-4" data-wow-delay=".2s">
              <h2 className="relative pb-5 text-2xl font-semibold text-black sm:text-[28px] dark:text-white">
                Related Articles
              </h2>
              <span className="bg-primary mb-10 inline-block h-[2px] w-20"></span>
            </div>
